...
class DirectChatWidgetWeb extends StatefulWidget {
  @override
  State<DirectChatWidgetWeb> createState() => _DirectChatWidgetWebState();
}

class _DirectChatWidgetWebState extends State<DirectChatWidgetWeb> {
  ...
  Future<void> initData() async {
    dynamic response =
        await _appService.getChatRoomByID(id: _appService.selectedChatRoomID);
    if (response is String || response == null) {
      setState(() {
        _appService.selectedChatRoomData = null;
        _appService.selectedChatUser = null;
      });
    } else {
      dynamic response2 = await _appService.getRequestById(
          requestID: response.data['room']['offer']['request_id']);
      if (response2 is String || response2 == null) {
        _appService.selectedJobData = null;
      } else {
        _appService.selectedJobData = response2.data;
      }
      if (response.data['room']['offer']['user_id'] != _appService.user['id']) {
        dynamic response1 = await _appService.getUserProfile(
            userID: response.data['room']['offer']['user_id']);

        if (response1 is String || response1 == null) {
          _appService.selectedChatUser = null;
        } else {
          _appService.selectedChatUser = response1.data['data'];
        }
      } else {
        if (_appService.selectedJobData != null) {
          dynamic response3 = await _appService.getUserProfile(
              userID: _appService.selectedJobData['user_id']);

          if (response3 is String || response3 == null) {
            _appService.selectedChatUser = null;
          } else {
            _appService.selectedChatUser = response3.data['data'];
          }
        } else {
          _appService.selectedChatUser = null;
        }
      ...
      _appService.selectedChatRoomData = response.data['room'];
    ...
    try {
      ...
      channel!.listen((message) async {
        ...
        if (jsonDecodedMessage['type'] == "video") {
          final file =
              File(jsonDecodedMessage['data']['meta_json']['videos'][0]['url']);
          ...
          final message = types.AudioMessage(
            ...
            id: jsonDecodedMessage['data']['id'].toString(),
            name: path.basename(file.path),
            size: jsonDecodedMessage['data']['meta_json']['videos'][0]['size'],
            uri: file.path,
          );
          _addMessage(message);
          if (jsonDecodedMessage['data']['user_id'] != _appService.user['id']) {
    ...
    return PreferredSize(
      child: Container(
        child: AppBar(
            ...
            elevation: 0.0,
            actions: [],
            title: _appService.selectedChatUser != null
                ? Row(
                    children: [
                      InkWell(
                          onTap: () async {
                          ...
                          child: Container(
                              child: _appService.selectedChatUser['avatar'] ==
                                          null ||
                      ...
                      Column(
                        ...
                        children: [
                          InkWell(
                              onTap: () {
                                if (_appService.selectedChatRoomData['offer']
                                        ['user_id'] !=
                                    _appService.user['id']) {
                          ...
                          SizedBox(
                            ...
                            child: isTyping
                                ...
                                : Text(
                                    ...
                                    style: TextStyle(
                                      color: _appService.selectedChatUser[
                                                      'is_online'] !=
                                                  null &&
                                              _appService.selectedChatUser[
                                                  'is_online'] &&
  ...
  Widget workDirectChatWidgetWeb(BuildContext context) {
                              ...
                              Chat(
                                ...
                                theme: context
                                    ...
                                    ? DefaultChatTheme(
                                        ...
                                        receivedMessageBodyTextStyle:
                                            TextStyle(color: Colors.white),
                                        primaryColor: Color.fromRGBO(5, 163,
                                            231, 1), // send message backgrou
                                        secondaryColor: Colors.black)
                                    : DarkChatTheme(
                                        backgroundColor:
                                            Color.fromARGB(255, 20, 20, 21),
                                        messageBorderRadius: 10,
                                        primaryColor:
                                            Color.fromRGBO(5, 163, 231,
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        InkWell(
                                            ...
                                            onTap: () async {
                                              if (_appService.selectedJobData['phone'] != null) if (!kIsWeb &&
                                                  (Platform.isAndroid ||
                                                      Platform.isIOS))
                                                await FlutterPhoneDirectCaller
                                                    .callNumber(_appService
                                                            .selectedJobData[
                                                        'phone']);
                                            }),
                                      ],
