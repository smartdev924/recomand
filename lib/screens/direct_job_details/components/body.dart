...
class _BodyState extends State<Body> {
  ...
  void initState() {
    ...
    if (_appService.user != null) getJobDataByID();
  }
  ...
  Future<void> getJobDataByID() async {
    if (_appService.deeplinkRequestID != '') {
      dynamic response = null;
      if (_appService.user != null) {
        response = await _appService.getRequestById(
            requestID: int.parse(_appService.deeplinkRequestID));
      } else {
        response = await _appService.getRequestInfoId(
            requestID: int.parse(_appService.deeplinkRequestID));
      }
      _appService.deeplinkRequestID = '';
      if (response is String || response == null) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_appStateListener.isInForeground &&
        _appService.deeplinkRequestID != '') {
      setState(() {
        isLoaded = false;
      });
    ...
    return !_appStateListener.isInForeground
        ? Container()
        : isLoaded
            ? _appService.selectedJobData == null
                ? Container()
                : Scaffold(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if (_appService.selectedJobData['files'] != null &&
                                _appService.selectedJobData['files'].length !=
                                    0)
                              image_carousel,
                            if (_appService.selectedJobData['customer'] !=
                                    null &&
                                _appService.loginState == false)
                              Column(
                                children: [
                                  Divider(
                                    thickness: 1,
                    ...
                    bottomNavigationBar: Container(
                        ...
                        child: _appService.user['is_worker'] == true
                            ? _appService.selectedJobData['have_offers'] == null
                                ? DefaultButton(
                                ...
                                : DefaultButton(
                                    ...
                                    press: () async {
                                      dynamic response =
                                          await _appService.getOfferById(
                                              offerID:
                                                  _appService.selectedJobData[
                                                      'have_offers']);
                                      if (response is String ||
                                        ...
                                        _appService.selectedOfferToUpdate =
                                            response.data;
                                        GoRouter.of(context).pushNamed(
